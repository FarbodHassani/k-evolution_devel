    ...
    for (xField.first(); xField.test(); xField.next())
      {
        ...
        if(method==1) // method=1 Turn on vector elliptic
        {
          if (non_linearity ==1)
			...
			//////////////////////////
			template <class FieldType>
			void update_pi_k( double dtau, double dx,double a, Field<FieldType> & phi, Field<FieldType> & phi_old, Field<FieldType> & chi,Field<FieldType> & chi_old, Field<FieldType> & pi_k, Field<FieldType> & zeta_half , Field<FieldType> & zeta_integer, double Omega_fld ,double w, double cs2, double Hcon, double  H_prime, int non_linearity)
			{

        double Gradphi_Gradpi, Gradpsi_Gradpi, Gradpi_Gradpi, GradPsiZeta_Gradpi, Dx_psi, Dy_psi, Dz_psi;
        double C1, C2, C3, psi, psi_old, psi_prime, phi_prime, Laplacian_pi, zeta_old_half, LinTerms, NL_terms, NL_Rel;
        ...
        double Coeff1 = 1./(1. + Hcon * dtau/2.);
            ...
            phi_prime= (( phi(x) - phi_old(x) )) / dtau; //phi'(n)
            phi_prime_old = ((phi_old(x) -  phi_older(x) ) ) / dtau;//phi'(n-1)
            phi_prime_half= 3./2. * (phi_prime) - phi_prime_old/2. //\Phi'(n+1/2) = \Phi'(n) +\Phi''(n) dtau
                                                                   // \Phi'(n+1/2) = 3/2 \Phi'(n) - \Phi'(n-1)/2
            ...
            //*****************************************
            //Calculating pi(n+1/2) to use in zeta'(n+1/2)
            //*****************************************
            pi_k_half = (pi_k(x) + pi_k_old)/2.;
            //**************************************************************************
            //zeta'(n+1/2) =3. * Hcon * ( w * zeta_half(n+1/2) which is already updated)
            //Look we need pi(n+1/2) since we have update pi(n+1) before this by saving pi(n)
            //we take the average pi(n+1/2) = pi(n+1) + pi(n)/2
            //**************************************************************************
            zeta_prime_half =
            /*Linear(1,2,3)*/      + 3. * Hcon * ( w * zeta_half(x) + cs2 * psi_half ) - C2 * pi_k_half
            /*Linear(4,5)*/        + 3. * cs2 * phi_prime_half + cs2 * Laplacian_pi
            /*Non-linear(1,2)*/    + 2. * cs2 * phi(x) * Laplacian_pi - (1. - cs2) * psi * Laplacian_pi
            /*Non-linear(3)  */    - 3. * cs2 * Hcon * (1. + w) * pi_k(x) * Laplacian_pi
      ...
      // updating zeta_half from n-1/2 to n+1/2 by zeta'(n)
			template <class FieldType>
			void update_zeta(double dtau, double dx,double a, Field<FieldType> & phi, Field<FieldType> & phi_old, Field<FieldType> & chi,Field<FieldType> & chi_old, Field<FieldType> & pi_k, Field<FieldType> & zeta_half , Field<FieldType> & zeta_integer, double Omega_fld ,double w, double cs2, double Hcon, double H_prime, int non_linearity )
			  {
        double Gradphi_Gradpi, Gradpsi_Gradpi, Gradpi_Gradpi, GradPsiZeta_Gradpi, Dx_psi, Dy_psi, Dz_psi;
			  double C1, C2, C3, psi, psi_old, psi_prime, phi_prime, Laplacian_pi, zeta_old_half, LinTerms, NL_terms, NL_Rel;
        //Since a_kess is at n so H_prime is at n which is needed to calculate zeta(n+1/2)
        ...
        C3 = (2. + 3. * w + cs2 ) *  Hcon/2.;
          ...
          // TESTS:
          //***************************************
          // cout<<"Term 0: "<< - C2 * pi_k(x)<<" Term1: "<<- (1. - cs2) * phi(x) * Laplacian_pi <<" Term2: "<<-3. * cs2 * Hcon * (1. + w) * pi_k(x) * Laplacian_pi << " Term 3: "<<cs2 * Gradphi_Gradpi<<" Term 4: "<<(2. * cs2 -1.) * Gradpsi_Gradpi<<" Term 5: "<<C3 * Gradpi_Gradpi <<"Term 6: "<<2. * (1. - cs2) * GradPsiZeta_Gradpi<<endl;

          //******************************************
          // NonLinear terms contribution in the zeta'
          //******************************************
          // LinTerms = 3. * Hcon * ( w * zeta_integer(x) + cs2 * psi ) - C2 * pi_k(x)
          // /*Linear(4,5)*/        + 3. * cs2 * phi_prime + cs2 * Laplacian_pi  ;
          // NL_terms =              (+ 2. * cs2 * phi(x) * Laplacian_pi - (1. - cs2) * psi * Laplacian_pi
          // /*Non-linear(3)  */    - 3. * cs2 * Hcon * (1. + w) * pi_k(x) * Laplacian_pi
          // /*Non-linear(4)  */    +(1. - cs2) * (zeta_half(x)/2. + psi) * Laplacian_pi
          // /*Non-linear(5,6,7)*/  - cs2 * Gradphi_Gradpi + (2. * cs2 -1.) * Gradpsi_Gradpi - C3 * Gradpi_Gradpi
          // /*Non-linear(8)  */    + 2. * (1. - cs2) * GradPsiZeta_Gradpi);
          // NL_Rel=  abs(NL_terms - LinTerms)/LinTerms;
          // /*Linear(1,2,3)*/
          // cout<<"NL_REL: "<<NL_Rel<<endl;
          //***************************************
          //***************************************
          //***************************************
          ...
          if (non_linearity ==1)
            ...
            int n_correcor_steps=12, numerator;
            numerator=0;
            //***********************
            ...
            for (int i=1; i<n_correcor_steps+1; i++)
              ...
              //***********************************************************************************
              if (abs(zeta_predictor_int_n0 - zeta_integer(x))/zeta_integer(x) <Max_error) break;
               ...
               // if (numerator==n_correcor_steps){if(parallel.isRoot()){ cout << "Zeta_last: "<< zeta_predictor_int_n0<< " Zeta_next: "<<zeta_integer(x)<<" Rel Dif: " <<2.*abs(zeta_integer(x)-zeta_predictor_int_n0)/(zeta_integer(x)+zeta_predictor_int_n0) <<'\n';}}
               numerator++;
              }
            if (numerator==n_correcor_steps){if(parallel.isRoot()){ cout << "\033[1;31mbold WARNING: PRECISION ERROR ON KESSENCE FIELD ZETA, More than expected\033[0m\n" <<" The Rel error: " <<2.*abs(zeta_integer(x)-zeta_predictor_int_n0)/(zeta_integer(x)+zeta_predictor_int_n0) <<'\n';}}
            }
          }
        }
