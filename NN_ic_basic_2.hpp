...
void loadTransferFunctions(const char * filename, gsl_spline * & tk_delta, gsl_spline * & tk_theta, const char * qname, const double boxsize, const double h)
{
	...
	if (parallel.grid_rank()[0] == 0) // read file
	{
		...
		if (kcol < 0 || dcol < 0 || tcol < 0)
		{
            ...
            cerr << " 1: " << kcol << "  2:"<<dcol<< "  3:" << tcol<<endl;
			fclose(tkfile);
			free(k);
			free(tk_d);
			free(tk_t);
...
void generateIC_basic(metadata & sim, icsettings & ic, cosmology & cosmo, const double fourpiG, Particles<part_simple,part_simple_info,part_simple_dataType> * pcls_cdm, Particles<part_simple,part_simple_info,part_simple_dataType> * pcls_b, Particles<part_simple,part_simple_info,part_simple_dataType> * pcls_ncdm, double * maxvel, Field<Real> * phi, Field<Real> * pi_k, Field<Real> * chi, Field<Real> * Bi, Field<Real> * source, Field<Real> * Sij, Field<Cplx> * scalarFT, Field<Cplx> * BiFT, Field<Cplx> * SijFT, PlanFFT<Cplx> * plan_phi,PlanFFT<Cplx> * plan_pi_k, PlanFFT<Cplx> * plan_chi, PlanFFT<Cplx> * plan_Bi, PlanFFT<Cplx> * plan_source, PlanFFT<Cplx> * plan_Sij)
{
	int i, j, p;
...
//    Site x(pi_k->lattice());
	...
	for (p = 0; p < cosmo.num_ncdm; p++)	// initialization of non-CDM species
	{
        ...
        for (i = 0; i < tk_d3->size; i++)
            temp1[i] = -3. * pkspline->y[i] / pkspline->x[i] / pkspline->x[i] - tk_d3->y[i] * M_PI * sqrt(Pk_primordial(tk_d3->x[i] * cosmo.h / sim.boxsize, ic) / tk_d3->x[i]) / tk_d3->x[i];
        
        tk_d3 = gsl_spline_alloc(gsl_interp_cspline, pkspline->size);
        tk_t3 = gsl_spline_alloc(gsl_interp_cspline, pkspline->size);
        gsl_spline_init(tk_d3, pkspline->x, temp1, pkspline->size);
        gsl_spline_init(tk_t3,  pkspline->x, temp2, pkspline->size);
        
        generateDisplacementField(*scalarFT, 0., tk_t3, (unsigned int) ic.seed, ic.flags & ICFLAG_KSPHERE, 0);
...
//        plan_source->execute(FFT_FORWARD);
//        plan_pi_k->execute(FFT_BACKWARD);	// pi_k now contains the displacement
//        pi_k->updateHalo();
//        gsl_spline_free(tk_d1);
//        generateRealization(scalarFT, 0., pkspline, (unsigned int) ic.seed, ic.flags & ICFLAG_KSPHERE,0);
