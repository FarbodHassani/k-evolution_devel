...
int project_halos(Field<Real> * halodens1, Field<Real> * halodens2, float * posdata, const int count, const double boxsize, int seed);
...
int main(int argc, char **argv)
{
	...
	int n = 0;
	int m = 0;
	int i, count = 0, tot = 0, tot1 = 0, mcount = 0, jkseed = 23;
	int metaread = 0;
	int numpts = 0;
	int numbins = 0;
	icsettings ic;
	...
	COUT << " Halo power spectrum tool" << endl;
	COUT << " Ngrid = " << numpts << "; Pk bins = " << numbins << "; halo mass limit = " << masslimit << "; seed = " << ic.seed << "; jackknife = " << jkseed << endl << endl;
	...
	Field<Real> halodens1;
	Field<Real> halodens2;
	Field<Real> phi;
	Field<Cplx> scalarFT;
	Field<Cplx> scalarFT2;

	halodens1.initialize(lat,1);
	halodens2.initialize(lat,1);
	scalarFT.initialize(latFT,1);
	scalarFT2.initialize(latFT,1);
	PlanFFT<Cplx> plan_halodens1(&halodens1, &scalarFT);
	PlanFFT<Cplx> plan_halodens2(&halodens2, &scalarFT2);
#ifdef READ_PHIFILE
	...
	projection_init(&halodens1);
	projection_init(&halodens2);
	...
	if (parallel.isRoot())
	{
		...
		while (!feof(fileptr) && !ferror(fileptr))
		{
			...
			else
			{
				...
				if (count == CHUNKSIZE)
				{
					parallel.broadcast<int>(count, 0);
					parallel.broadcast<float>(posdata, 3*count, 0);
					tot1 += project_halos(&halodens1, &halodens2, posdata, count, boxsize, jkseed);
					tot += count;
					count = 0;
				}
			}
		...
		COUT << " " << tot << " halos used. Computing power spectrum..." << endl;
	}
	else
	{
		parallel.broadcast<double>(a, 0);
	...
	projection_T00_comm(&halodens1);
	projection_T00_comm(&halodens2);
	...
	for (x.first(); x.test(); x.next())
	{
		...
		halodens1(x) *= (1. + 3. * temp / 64.) / (Real) tot1;
		halodens2(x) *= (1. + 3. * temp / 64.) / (Real) (tot-tot1);
	}
#else
	...
	for (x.first(); x.test(); x.next())
	{
		halodens1(x) *= (1. + 3. * phi(x)) / (Real) tot1;
		halodens2(x) *= (1. + 3. * phi(x)) / (Real) (tot-tot1);
	}
#endif

	plan_halodens1.execute(FFT_FORWARD);
	plan_halodens2.execute(FFT_FORWARD);
	...
	sprintf(filename, "%s_halopk.dat", filebase);
	writePowerSpectrum(kbin, power, kscatter, pscatter, occupation, numbins, boxsize, 2. * M_PI * M_PI, filename, "power spectrum of halos", a);
...
int project_halos(Field<Real> * halodens1, Field<Real> * halodens2, float * posdata, const int count, const double boxsize, int seed)
{
	Site x(halodens1->lattice());

	int i, xh, yh, zh, numpts, c = 0;
	...
	numpts = halodens1->lattice().size(0);
	...
	for (i = 0; i < count; i++)
	{
		...
		{
			(*fld)(x) += (1.-xr) * (1.-yr) * (1.-zr);
			(*fld)(x+0) += xr * (1.-yr) * (1.-zr);
			(*fld)(x+1) += (1.-xr) * yr * (1.-zr);
			(*fld)(x+2) += (1.-xr) * (1.-yr) * zr;
			(*fld)(x+0+1) += xr * yr * (1.-zr);
			(*fld)(x+0+2) += xr * (1.-yr) * zr;
			(*fld)(x+1+2) += (1.-xr) * yr * zr;
			(*fld)(x+0+1+2) += xr * yr * zr;
		}
	}
