...
#pragma omp parallel for
	...
	else
	{
		...
		for (int n = 0; n < Nsource; n++)
		{
			if((c = fscanf(infile, " %lf %lf %lf %*f %*e %*e %*e %*f %*f", source_direction+(2*n), source_direction+(1+2*n), dummy_read)) != 3)
			{
				cout << COLORTEXT_RED << " error" << COLORTEXT_RESET << ": could not read source data from catalog for source number " << n << ", number of items read = " << c << endl;
				return -1;
			}
...
#pragma omp critical(evala)
{
		...
		else
		{
			...
			fprintf(outfile, "# theta      phi        1+z_obs      D_A          Re(e)          Im(e)           residual       mu'             phi'\n");
			for (int n = 0; n < Nsource; n++)
			{
				if (photon[n].stop > 0)
					fprintf(outfile, "  %f   %f   %.8lf   %.8lf   %e   %e   %e   %.10lf   %.10lf\n", source_direction[2*n], (source_direction[1+2*n] < 0.) ? source_direction[1+2*n] + 2.*M_PI : source_direction[1+2*n], photon[n].lnk0, photon[n].DA, photon[n].ellip[0], photon[n].ellip[1], sqrt((photon[n].pos[0]-source_pos[3*n])*(photon[n].pos[0]-source_pos[3*n]) + (photon[n].pos[1]-source_pos[1+3*n])*(photon[n].pos[1]-source_pos[1+3*n]) + (photon[n].pos[2]-source_pos[2+3*n])*(photon[n].pos[2]-source_pos[2+3*n]))*sim.numpts, sin(source_direction[2*n])*(cos(source_direction[1+2*n])*coordsys[3]*coordsys[0] + sin(source_direction[1+2*n])*coordsys[3]*coordsys[1]) + cos(source_direction[2*n])*coordsys[2], atan2(sin(source_direction[2*n])*(coordsys[0]*sin(source_direction[1+2*n])-coordsys[1]*cos(source_direction[1+2*n])), sin(source_direction[2*n])*coordsys[2]*(coordsys[1]*sin(source_direction[1+2*n])+coordsys[0]*cos(source_direction[1+2*n])) - cos(source_direction[2*n])*coordsys[3]));
				else
					fprintf(outfile, "  %f   %f   -1.0000000   -1.0000000   0.000000e+00   0.000000e+00   -1.000000e+00   %.10lf   %.10lf\n", source_direction[2*n], (source_direction[1+2*n] < 0.) ? source_direction[1+2*n] + 2.*M_PI : source_direction[1+2*n], sin(source_direction[2*n])*(cos(source_direction[1+2*n])*coordsys[3]*coordsys[0] + sin(source_direction[1+2*n])*coordsys[3]*coordsys[1]) + cos(source_direction[2*n])*coordsys[2], atan2(sin(source_direction[2*n])*(coordsys[0]*sin(source_direction[1+2*n])-coordsys[1]*cos(source_direction[1+2*n])), sin(source_direction[2*n])*coordsys[2]*(coordsys[1]*sin(source_direction[1+2*n])+coordsys[0]*cos(source_direction[1+2*n])) - cos(source_direction[2*n])*coordsys[3]));
			}
			fclose(outfile);
		}
