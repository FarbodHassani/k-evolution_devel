...
void addAttribute(string filename, string attr_name, double attr)
{
	if(parallel.isRoot())
	{
		hid_t file_id,faplist_id;
		hid_t status,root_id,attr_id,attr_type, dataspace_id;

	
//		faplist_id = H5Pcreate(H5P_FILE_ACCESS);
//		status = H5Pset_falp_stdio(faplist_id);
		file_id = H5Fopen(filename.c_str(),H5F_ACC_RDWR,H5P_DEFAULT);
		root_id = H5Gopen(file_id,"/",H5P_DEFAULT);


		dataspace_id = H5Screate(H5S_SCALAR);
		attr_id = H5Acreate(root_id,attr_name.c_str(),H5T_NATIVE_DOUBLE,dataspace_id,H5P_DEFAULT,H5P_DEFAULT);
		status = H5Awrite(attr_id,H5T_NATIVE_DOUBLE,&attr);
